...
export const gotItemsAtom = atom(get => {
  const filtered = Object.values(get(storeItemsAtom)).filter(item => item.got) as GotItem[];
...
const applyActionAtom = atom(null,
...
export const applyAndSyncAtom = atom(null,
...
export const appendNewItemAtom = atom(null,
  (get, set) => {
...
export const handleKeydownAtom = atom(null,
  (get, set, event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.code == "Enter") {
      ...
      if (!val) {
        ...
        set(applyAndSyncAtom, { op: "delete", table: "store_items", entity: { id } })
        set(focusIndexAtom, i => i - 1);
      }
    }
...
export const handleDragStartAtom = atom(null,
  (_get, set) => {
    set(focusIndexAtom, -1);
  })

export const handleDragEndAtom = atom(null,
  (get, set, event: any) => {
...
export const handleCheckboxAtom = atom(null,
...
export const handleTextboxAtom = atom(null,
  (_get, set, event: ChangeEvent) => {
    set(applyAndSyncAtom,
      {
        op: "edit",
