...
var usePathname = ({} = {}) =>
  useLocationProperty(currentPathname, currentPathname);
...
var useHashLocation =
  () => [C(subscribeToHashUpdates, currentHashLocation), hashNavigate];
...
function Route({ path, nest, match, ...renderProps }) {
  ...
  const [matches, routeParams, base] = match ??
  ...
  const children = base
    ? k(Router, { base }, createRouteElement(renderProps, params))
...
var flattenChildren = (children) =>
  Array.isArray(children)
    ? children.flatMap((c) =>
      flattenChildren(c && c.type === S ? c.props.children : c)
    )
    : [children];
