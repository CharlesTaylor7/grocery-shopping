import {
  ...
  x as h,
  y as _,
} from "./accessibility-BA1htokQ.js";
...
var CSS = /*#__PURE__*/ Object.freeze({
  ...
  Transform: {
    toString(transform) {
      if (!transform) return;
      return [CSS.Translate.toString(transform), CSS.Scale.toString(transform)]
        .join(" ");
    },
  },
  Transition: {
...
var defaultAnnouncements = {
  ...
  onDragOver(_ref2) {
    ...
    if (over) {
      return "Draggable item " + active.id + " was moved over droppable area " +
        over.id + ".";
    }
    return "Draggable item " + active.id +
  ...
  onDragEnd(_ref3) {
    ...
    if (over) {
      return "Draggable item " + active.id +
        " was dropped over droppable area " + over.id;
    }
    return "Draggable item " + active.id + " was dropped.";
  },
  onDragCancel(_ref4) {
    let { active } = _ref4;
    return "Dragging was cancelled. Draggable item " + active.id +
      " was dropped.";
  },
};
function Accessibility(_ref) {
  ...
  const markup = gn.createElement(
    gn.Fragment,
    null,
    gn.createElement(HiddenText, {
...
function getIntersectionRatio(entry, target) {
  ...
  if (left < right && top < bottom) {
    ...
    const intersectionRatio = intersectionArea /
...
function getScrollableAncestors(element, limit) {
  ...
  function findScrollableAncestors(node) {
    ...
    if (
      isDocument(node) && node.scrollingElement != null &&
...
function getScrollDirectionAndSpeed(
  ...
  if (!isTop && top <= scrollContainerRect.top + threshold.height) {
    ...
    !isBottom && bottom >= scrollContainerRect.bottom - threshold.height
  ) {
...
function scrollIntoViewIfNeeded(element, measure) {
  ...
  if (
    bottom <= 0 || right <= 0 || top >= window.innerHeight ||
      ...
      if (newCoordinates) {
        ...
        for (const scrollContainer of scrollableAncestors) {
          ...
          if (canScrollX && clampedCoordinates.x !== newCoordinates.x) {
            ...
            } else {scrollDelta.x = direction === KeyboardCode.Right
            ...
            const newScrollCoordinates = scrollContainer.scrollTop +
              coordinatesDelta.y;
            const canScrollToNewCoordinates = direction === KeyboardCode.Down &&
            ...
            } else {scrollDelta.y = direction === KeyboardCode.Down
...
var AbstractPointerSensor = class {
  ...
  attach() {
    const {
      events,
      props: { options: { activationConstraint, bypassActivationConstraint } },
    } = this;
    ...
    if (activationConstraint) {
      if (
        bypassActivationConstraint != null && bypassActivationConstraint({
...
function useAutoScroller(_ref) {
  ...
  const sortedScrollableAncestors = T(
    () =>
      order === TraversalOrder.TreeOrder
...
function useDroppableMeasuring(containers, _ref) {
  ...
  const droppableRects = useLazyMemo((previousValue) => {
    ...
    if (
      ...
      containersRef.current !== containers || queue != null
    ) {
      const map = /* @__PURE__ */ new Map();
      for (let container of containers) {
        if (!container) continue;
        if (
          queue && queue.length > 0 && !queue.includes(container.id) &&
  ...
  h(() => {
    if (
      disabled || typeof frequency !== "number" || timeoutId.current !== null
    ) return;
  ...
  const mutationObserver = useMutationObserver({
    callback(records) {
      ...
      for (const record of records) {
        ...
        if (
          type === "childList" && target instanceof HTMLElement &&
...
function useScrollableAncestors(node) {
  ...
  const ancestors = useLazyMemo((previousValue) => {
    ...
    if (
      previousValue && previousValue !== defaultValue$1 && node &&
...
var DroppableContainersMap = class extends Map {
  get(id) {
    var _super$get;
    return id != null
      ? (_super$get = super.get(id)) != null ? _super$get : void 0
      : void 0;
  }
  toArray() {
  ...
  getNodeFor(id) {
    ...
    return (_this$get$node$curren = (_this$get = this.get(id)) == null
        ? void 0
...
var DndContext = /*#__PURE__*/ N(function DndContext(_ref) {
  ...
  const active = T(() => {
    ...
    return activeId != null
      ? {
        id: activeId,
        data: (_node$data = node == null ? void 0 : node.data) != null
          ? _node$data
          : defaultData,
        rect: activeRects,
      }
  ...
  const enabledDroppableContainers = T(() => droppableContainers.getEnabled(), [
    droppableContainers,
  ]);
      ...
      return async function handler() {
        ...
        if (active && scrollAdjustedTranslate) {
          ...
          if (type === Action.DragEnd && typeof cancelDrop === "function") {
            if (await Promise.resolve(cancelDrop(event))) {
  ...
  const activators = useCombineActivators(
    ...
    q((handler, sensor) => {
      return (event, active) => {
        ...
        if (
          activeRef.current !== null || !activeDraggableNode ||
  ...
  h(() => {
    ...
    if (
      !active || activeRef.current == null || !activatorEvent ||
      !scrollAdjustedTranslate
    ) return;
  ...
  function getAutoScrollerOptions() {
    ...
    const autoScrollGloballyDisabled = typeof autoScroll === "object"
      ...
      : autoScroll === false;
    const enabled = isInitialized && !activeSensorDisablesAutoscroll &&
...
function useDroppable(_ref) {
  ...
  const { active, dispatch, over, measureDroppableContainers } = x(
    InternalContext,
  );
...
export {
  ...
  DndContext as t,
  getClientRect as a,
  isKeyboardEvent as f,
  KeyboardCode as n,
  PointerSensor as r,
  useCombinedRefs as p,
  useDndContext as o,
  useDraggable as s,
  useDroppable as c,
  useIsomorphicLayoutEffect as m,
  useSensor as l,
  useSensors as u,
  useUniqueId as h,
};
