...
//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/utils/worldwide.js
var GLOBAL_OBJ = globalThis;

//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/utils/version.js
...
//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/utils/stacktrace.js
...
function isParameterizedString(wat) {
  return typeof wat === "object" && wat !== null &&
    "__sentry_template_string__" in wat && "__sentry_template_values__" in wat;
}
...
function fill(source, name, replacementFactory) {
  ...
  try {
    ...
    DEBUG_BUILD$2 &&
      debug.log(`Failed to replace method "${name}" in object`, source);
  }
}
function addNonEnumerableProperty(obj, name, value) {
  try {
    ...
    DEBUG_BUILD$2 &&
      debug.log(
        `Failed to add non-enumerable property "${String(name)}" to object`,
        obj,
      );
  }
}
...
var SENTRY_SKIP_NORMALIZATION = /* @__PURE__ */ Symbol.for(
  "sentry.skipNormalization",
);
var SENTRY_OVERRIDE_NORMALIZATION_DEPTH = /* @__PURE__ */ Symbol.for(
  "sentry.overrideNormalizationDepth",
);
...
function visit(
  ...
  if (
    value == null || ["boolean", "string"].includes(typeof value) ||
...
function createUnixTimestampInSecondsFunc() {
  ...
  return () => {
    return (timeOrigin + withRandomSafeContext(() => performance.now())) /
      ONE_SECOND_IN_MS;
  };
}
var _cachedTimestampInSeconds;
function timestampInSeconds() {
  return (_cachedTimestampInSeconds ??
    (_cachedTimestampInSeconds = createUnixTimestampInSecondsFunc()))();
}
...
function sessionToJSON(session) {
  return {
    ...
    timestamp: (/* @__PURE__ */ new Date(session.timestamp * 1e3))
      .toISOString(),
...
function derefWeakRef(ref) {
  if (!ref) return;
  if (
    typeof ref === "object" && "deref" in ref && typeof ref.deref === "function"
  ) {
    try {
      return ref.deref();
    } catch {
...
var chainAndCopyPromiseLike = (original, onSuccess, onError) => {
  ...
  return isActualPromise(chained) && isActualPromise(original)
    ? chained
    : copyProps(original, chained);
};
...
//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/asyncContext/stackStrategy.js
...
function getAsyncContextStack() {
  ...
  return sentry.stack = sentry.stack ||
    new AsyncContextStack(getDefaultCurrentScope(), getDefaultIsolationScope());
}
...
function isAttributeObject(maybeObj) {
  return typeof maybeObj === "object" && maybeObj != null &&
    !Array.isArray(maybeObj) && Object.keys(maybeObj).includes("value");
}
...
function getTypedAttributeValue(value) {
  ...
  const primitiveType = typeof value === "string"
    ? "string"
...
function getTraceContextFromScope(scope) {
  ...
  const { traceId, parentSpanId, propagationSpanId } = scope
    .getPropagationContext();
  const traceContext = {
    trace_id: traceId,
...
//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/semanticAttributes.js
var SEMANTIC_ATTRIBUTE_SENTRY_SOURCE = "sentry.source";
var SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE = "sentry.sample_rate";
var SEMANTIC_ATTRIBUTE_SENTRY_PREVIOUS_TRACE_SAMPLE_RATE =
...
function getCapturedScopesOnSpan(span) {
  ...
  return {
    ...
    isolationScope: derefWeakRef(
      spanWithScopes[ISOLATION_SCOPE_ON_START_SPAN_FIELD],
    ),
  };
}

//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/utils/baggage.js
...
function parseBaggageHeader(baggageHeader) {
  if (
    !baggageHeader || !isString(baggageHeader) && !Array.isArray(baggageHeader)
  ) return;
...
function baggageHeaderToObject(baggageHeader) {
  return baggageHeader.split(",").map((baggageEntry) => {
    ...
    if (eqIdx === -1) return [];
    return [baggageEntry.slice(0, eqIdx), baggageEntry.slice(eqIdx + 1)].map(
      (keyOrValue) => {
        try {
...
//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/utils/spanUtils.js
...
function convertSpanLinksForEnvelope(links) {
  if (links && links.length > 0) {
    return links.map((
      { context: { spanId, traceId, traceFlags, ...restContext }, attributes },
    ) => ({
      span_id: spanId,
      trace_id: traceId,
...
function shouldIgnoreSpan(span, ignoreSpans) {
  ...
  for (const pattern of ignoreSpans) {
    ...
    const hasAttributes = !!pattern.attributes &&
      Object.keys(pattern.attributes).length > 0;
    if (!pattern.name && !pattern.op && !hasAttributes) continue;
    const nameMatches = pattern.name
      ? span.description && isMatchingPattern(span.description, pattern.name)
      : true;
    const opMatches = pattern.op
      ? span.op && isMatchingPattern(span.op, pattern.op)
      : true;
...
function _matchesAttributeValue(actual, pat) {
  if (
    ...
    (typeof pat === "string" || pat instanceof RegExp)
...
var NON_RECORDING_SPAN_FIELD = /* @__PURE__ */ Symbol.for(
  "sentry.nonRecordingSpan",
);
...
//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/tracing/dynamicSamplingContext.js
...
function getDynamicSamplingContextFromSpan(span) {
  ...
  const rootSpanSampleRate = traceState?.get("sentry.sample_rate") ??
  ...
  function applyLocalSampleRateToDsc(dsc2) {
    if (
      typeof rootSpanSampleRate === "number" ||
      typeof rootSpanSampleRate === "string"
  ...
  if (
    isNonRecordingRoot &&
    (!hasSpansEnabled(client.getOptions()) || isIgnoredRoot)
  ) {
  ...
  const source = rootSpanAttributes[SEMANTIC_ATTRIBUTE_SENTRY_SOURCE] ??
    rootSpanAttributes["sentry.segment.name.source"];
  const name = rootSpanJson.description;
  if (source !== "url" && name) dsc.transaction = name;
  if (hasSpansEnabled()) {
    dsc.sampled = String(spanIsSampled(rootSpan));
    dsc.sample_rand = traceState?.get("sentry.sample_rand") ??
      getCapturedScopesOnSpan(rootSpan).scope?.getPropagationContext()
        .sampleRand.toString();
  }
...
function isStreamedBeforeSendSpanCallback(callback) {
  return !!callback && typeof callback === "function" &&
    "_streamed" in callback && !!callback._streamed;
}
...
function _notifyEventProcessors(event, hint, processors, index) {
  ...
  DEBUG_BUILD$2 && result === null &&
    debug.log(`Event processor "${processor.id || "?"}" dropped event`);
  if (isThenable(result)) {
    return result.then((final) =>
      _notifyEventProcessors(final, hint, processors, index + 1)
    );
  }
...
function getFilenameToDebugIdMap(stackParser) {
  ...
  const sentryDebugIdKeys = sentryDebugIdMap
    ? Object.keys(sentryDebugIdMap)
    : [];
  const nativeDebugIdKeys = nativeDebugIdMap
    ? Object.keys(nativeDebugIdMap)
    : [];
  if (
...
function prepareEvent(options, event, hint, scope, client, isolationScope) {
  ...
  return (hint.data && hint.data.__sentry__ === true
    ? resolvedSyncPromise(prepared)
    : notifyEventProcessors(eventProcessors, prepared, hint)).then((evt) => {
...
//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/api.js
...
function afterSetupIntegrations(client, integrations) {
  for (const integration of integrations) {
    if (integration?.afterAllSetup) integration.afterAllSetup(client);
  }
}
...
function spanJsonToSerializedStreamedSpan(span) {
  return streamedSpanJsonToSerializedSpan({
    ...
    status: !span.status || span.status === "ok" || span.status === "cancelled"
      ? "ok"
      : "error",
    is_segment: false,
...
function extractGenAiSpansFromEvent(event, client) {
  if (
    event.type !== "transaction" || !event.spans?.length ||
  ...
  const inferSetting = client.getDataCollectionOptions().userInfo
    ? "auto"
    : "never";
  return [{
    type: "span",
...
var SENTRY_BUFFER_FULL_ERROR = /* @__PURE__ */ Symbol.for(
...
function makePromiseBuffer(limit = 100) {
  ...
  function drain(timeout) {
    ...
    const promises = [
      drainPromise,
      new Promise((resolve) =>
        safeUnref(setTimeout(() => resolve(false), timeout))
      ),
    ];
    ...
    for (const limit of rateLimitHeader.trim().split(",")) {
      ...
      if (!categories) updatedRateLimits.all = now + delay;
          ...
          if (category === "metric_bucket") {
            if (
              !namespaces || namespaces.split(";").includes("custom")
  ...
  function send(envelope) {
    ...
    const recordEnvelopeLoss = (reason) => {
      if (envelopeContainsItemType(filteredEnvelope, ["client_report"])) {
        DEBUG_BUILD$2 &&
          debug.warn(
            `Dropping client report. Will not send outcomes (reason: ${reason}).`,
          );
        return;
      }
    ...
    const requestTask = () =>
      makeRequest({ body: serializeEnvelope(filteredEnvelope) }).then(
        (response) => {
          ...
          if (
            DEBUG_BUILD$2 && response.statusCode !== void 0 &&
        ...
        (error) => {
          ...
          DEBUG_BUILD$2 &&
            debug.error("Encountered error running transport request:", error);
          throw error;
        },
      );
...
function convertTransactionEventToSpanJson(event) {
  const { trace_id, parent_span_id, span_id, status, origin, data, op } =
...
var MISSING_RELEASE_FOR_SESSION_ERROR =
  "Discarded session because of missing or non-string release";
var INTERNAL_ERROR_SYMBOL = /* @__PURE__ */ Symbol.for("SentryInternalError");
var DO_NOT_SEND_EVENT_SYMBOL = /* @__PURE__ */ Symbol.for(
...
function setupWeightBasedFlushing(
  ...
  client.on(afterCaptureHook, (item) => {
    ...
    if (weight >= 8e5) flushFn(client);
      ...
      const flushInterval = client.getOptions()._flushInterval ??
...
var Client = class {
  constructor(options) {
    ...
    this._promiseBuffer = makePromiseBuffer(
      options.transportOptions?.bufferSize ?? DEFAULT_TRANSPORT_BUFFER_SIZE,
    );
    ...
    this._options.enableLogs = this._options.enableLogs ??
  ...
  captureEvent(event, hint, currentScope) {
    const eventId = uuid4();
    if (
      hint?.originalException && checkOrSetAlreadyCaught(hint.originalException)
    ) {
      DEBUG_BUILD$2 && debug.log(ALREADY_SEEN_ERROR);
      return eventId;
    }
  ...
  sendSession(session) {
    ...
    if ("aggregates" in session) {
      ...
      sessionAttrs.environment = sessionAttrs.environment ||
  ...
  on(hook, callback) {
    const hookCallbacks = this._hooks[hook] =
      this._hooks[hook] || /* @__PURE__ */ new Set();
  ...
  _prepareEvent(event, hint, currentScope, isolationScope) {
    ...
    return prepareEvent(
      ...
      evt.sdkProcessingMetadata = {
        dynamicSamplingContext: getDynamicSamplingContextFromScope(
          this,
          currentScope,
        ),
        ...evt.sdkProcessingMetadata,
    ...
    if (DEBUG_BUILD$2 && isErrorEvent(event)) {
      debug.log(
        `Captured error event \`${
          getPossibleEventMessages(event)[0] || "<unknown>"
        }\``,
      );
    }
  ...
  _processEvent(event, hint, currentScope, isolationScope) {
    ...
    const dataCategory = getDataCategoryByType(event.type);
    return this._prepareEvent(event, hint, currentScope, isolationScope).then(
      (prepared) => {
        if (prepared === null) {
        ...
        return _validateBeforeSendResult(
          processBeforeSend(this, options, prepared, hint),
          beforeSendLabel,
        );
      },
    ).then((processedEvent) => {
      ...
      if (isTransaction) {
        const droppedSpanCount =
          (processedEvent.sdkProcessingMetadata?.spanCountBeforeProcessing ||
            0) - (processedEvent.spans ? processedEvent.spans.length : 0);
...
function processBeforeSend(client, options, event, hint) {
  ...
  const beforeSendSpan =
    !isStreamedBeforeSendSpanCallback(options.beforeSendSpan) &&
  ...
  if (isTransactionEvent(processedEvent)) {
    if (beforeSendSpan || ignoreSpans) {
      ...
      if (
        ignoreSpans?.length && shouldIgnoreSpan({
      ...
      if (processedEvent.spans) {
        ...
        const droppedSpans = processedEvent.spans.length -
...
function stripDataUrlContent(url, includeDataPrefix = true) {
  if (url.startsWith("data:")) {
    ...
    if (includeDataPrefix && dataStart !== -1) {
      ...
      dataPrefix = data.length > 10
        ? `${data.slice(0, 10)}... [truncated]`
        : data;
    }
...
function applySdkMetadata(options, name, names = [name], source = "npm") {
  const sdk = (options._metadata = options._metadata || {}).sdk =
...
var functionToStringIntegration = defineIntegration(
  _functionToStringIntegration,
);

//#endregion
//#region node_modules/.pnpm/@sentry+core@10.69.0/node_modules/@sentry/core/build/esm/integrations/eventFilters.js
...
Event: ${getEventDescription(event)}`,
    ...
    if (_isDeniedUrl(event, options.denyUrls)) {
      DEBUG_BUILD$2 &&
        debug.warn(`Event dropped due to being matched by \`denyUrls\` option.
Event: ${getEventDescription(event)}.
...
function _getEventFilterUrl(event) {
  try {
    const frames = [...event.exception?.values ?? []].reverse().find((value) =>
      value.mechanism?.parent_id === void 0 && value.stacktrace?.frames?.length
    )?.stacktrace?.frames;
    return frames ? _getLastValidUrl(frames) : null;
  } catch {
    DEBUG_BUILD$2 &&
      debug.error(`Cannot extract url for event ${getEventDescription(event)}`);
    return null;
  }
}
...
function hasSentryFetchUrlHost(error) {
  return isError(error) && "__sentry_fetch_url_host__" in error &&
  ...
  CONSOLE_LEVELS.forEach(function (level) {
    ...
    fill(GLOBAL_OBJ.console, level, function (originalConsoleMethod) {
      ...
      return function (...args) {
        ...
        const isFiltered = _filter.size && typeof firstArg === "string" &&
          stringMatchesSomePattern(firstArg, _filter);
        if (!isFiltered) {
...
function _isSameStacktrace(currentEvent, previousEvent) {
  ...
  for (let i = 0; i < previousFrames.length; i++) {
    ...
    if (
      frameA.filename !== frameB.filename || frameA.lineno !== frameB.lineno ||
      frameA.colno !== frameB.colno || frameA.function !== frameB.function
    ) return false;
  }
  return true;
}
  ...
  return {
    ...
    setup(client) {
      client.on("spanStart", (span) => {
        ...
        const conversationId = scopeData.conversationId ||
          isolationScopeData.conversationId;
        if (conversationId) {
          const { op, data: attributes, description: name } = spanToJSON(span);
          if (
            !op?.startsWith("gen_ai.") && !attributes["ai.operationId"] &&
            !name?.startsWith("ai.")
          ) return;
  ...
  fill(GLOBAL_OBJ, "fetch", function (originalFetch) {
    return function (...args) {
      ...
      return originalFetch.apply(GLOBAL_OBJ, args).then(async (response) => {
        ...
        const enhanceOption =
          getClient()?.getOptions().enhanceFetchErrorMessages ?? "always";
        if (
          enhanceOption !== false && error instanceof TypeError &&
...
function getHeadersFromFetchArgs(fetchArgs) {
  ...
  try {
    if (
      typeof optionsArgument === "object" && optionsArgument !== null &&
      "headers" in optionsArgument && optionsArgument.headers
...
function wrap(fn, options = {}) {
  ...
  const sentryWrapped = function (...args) {
    ...
    try {
      ...
      GLOBAL_OBJ._sentryWrappedDepth = (GLOBAL_OBJ._sentryWrappedDepth || 0) -
        1;
    }
  };
  try {
...
function extractType(ex) {
  ...
  if (!name && isWebAssemblyException(ex)) {
    return ex.message && Array.isArray(ex.message) && ex.message.length == 2
      ? ex.message[0]
...
function eventFromUnknownInput(
  ...
  if (isDOMError(exception) || isDOMException(exception)) {
    ...
    if ("stack" in exception) {
      ...
      if (
        attachStacktrace && syntheticException && firstException &&
...
function getNonErrorObjectExceptionValue(exception, { isUnhandledRejection }) {
  ...
  if (isEvent(exception)) {
    return `Event \`${
      getObjectClassName(exception)
    }\` (type=${exception.type}) captured as ${captureType}`;
  }
...
var BrowserClient = class extends Client {
  constructor(options) {
    ...
    applySdkMetadata(
      ...
      "browser",
      ["browser"],
      WINDOW$1.SENTRY_SDK_SOURCE || getSDKSource(),
    );
    super(opts);
...
function applyDefaultOptions(optionsArg) {
  return {
    release: typeof __SENTRY_RELEASE__ === "string"
      ? __SENTRY_RELEASE__
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser-utils@10.69.0/node_modules/@sentry/browser-utils/build/esm/types.js
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser-utils@10.69.0/node_modules/@sentry/browser-utils/build/esm/metrics/web-vitals/lib/whenIdleOrHidden.js
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser-utils@10.69.0/node_modules/@sentry/browser-utils/build/esm/htmlTreeAsString.js
var DEFAULT_MAX_STRING_LENGTH = 80;
var accessors = {};
try {
  if (typeof Node !== "undefined") {
    accessors.parentNode =
  ...
  if (typeof Element !== "undefined") {
    ...
    accessors.className =
      Object.getOwnPropertyDescriptor(Element.prototype, "className").get;
...
function _htmlElementAsString(el, keyAttrs) {
  ...
  const keyAttrPairs = keyAttrs?.length
    ? keyAttrs.filter((keyAttr) => _safeRead(el, "getAttribute", keyAttr)).map((
      keyAttr,
    ) => [keyAttr, _safeRead(el, "getAttribute", keyAttr)])
    : null;
    ...
    apply(originalOpen, xhrOpenThisArg, xhrOpenArgArray) {
      ...
      const onreadystatechangeHandler = () => {
        ...
        if (xhrOpenThisArg.readyState === 4) {
          ...
          xhrOpenThisArg.removeEventListener(
            "readystatechange",
      ...
      } else {xhrOpenThisArg.addEventListener(
          "readystatechange",
          onreadystatechangeHandler,
        );}
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser@10.69.0/node_modules/@sentry/browser/build/npm/esm/prod/stack-parsers.js
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser@10.69.0/node_modules/@sentry/browser/build/npm/esm/prod/integrations/breadcrumbs.js
      ...
      if (eventTargetOption) {
        (Array.isArray(eventTargetOption)
          ...
          : DEFAULT_EVENT_TARGET).forEach((target) =>
            _wrapEventTarget(target, _options)
          );
      }
    },
  };
};
var browserApiErrorsIntegration = defineIntegration(
...
function unregisterOriginalCallback(target, eventName, fn) {
  if (
    target && typeof target === "object" && "removeEventListener" in target &&
  ...
  return {
    ...
    setupOnce() {
      ...
      isolationScope.addScopeListener((scope) => {
        ...
        if (
          previousUser?.id !== maybeNewUser?.id ||
          previousUser?.ip_address !== maybeNewUser?.ip_address
        ) {
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser@10.69.0/node_modules/@sentry/browser/build/npm/esm/prod/integrations/culturecontext.js
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser@10.69.0/node_modules/@sentry/browser/build/npm/esm/prod/integrations/linkederrors.js
...
//#endregion
//#region node_modules/.pnpm/@sentry+browser@10.69.0/node_modules/@sentry/browser/build/npm/esm/prod/normalizeStringifyValue.js
...
function _isEmbeddedBrowserExtension() {
  ...
  return !(WINDOW$1 === WINDOW$1.top &&
    /^(?:chrome-extension|moz-extension|ms-browser-extension|safari-web-extension):\/\//
      .test(href));
}
...
function init(options = {}) {
  ...
  let defaultIntegrations = options.defaultIntegrations == null
