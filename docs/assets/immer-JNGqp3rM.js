...
function isDraftable(value) {
  ...
  return isPlainObject(value) || isArray(value) || !!value[DRAFTABLE] ||
    !!value[CONSTRUCTOR]?.[DRAFTABLE] || isMap(value) || isSet(value);
}
...
function finalize(rootScope, value) {
  ...
  if (!state.finalized_) {
    ...
    if (callbacks_) {
      while (callbacks_.length > 0) callbacks_.pop()(rootScope);
    }
...
function updateDraftInParent(parent, draftValue, finalizedValue, originalKey) {
  ...
  const locations = parent.draftLocations_.get(draftValue) ??
...
function generatePatchesAndFinalize(state, rootScope) {
  if (
    state.modified_ && !state.finalized_ &&
    (state.type_ === 3 || state.type_ === 1 && state.allIndicesReassigned_ ||
    ...
    target.callbacks_.push(function nestedDraftCleanup() {
      ...
      if (target.type_ === 3) {
        ...
        if (
          ...
          (target.assigned_.get(key) ?? false) === true && target.copy_
        ) {
          handleValue(
...
function handleValue(target, handledSet, rootScope) {
  ...
  if (
    isDraft(target) || handledSet.has(target) || !isDraftable(target) ||
    isFrozen(target)
  ) return target;
...
var objectTraps = {
  get(state, prop) {
    ...
    if (
      isArrayWithStringProp && state.operationMethod &&
  ...
  set(state, prop, value) {
    ...
    if (
      state.copy_[prop] === value &&
        (value !== void 0 || has(state.copy_, prop, state.type_)) ||
...
function isRelocatedBaseRef(state, prop, value) {
  if (
    ...
    state.assigned_?.get(prop) || !isDraftable(value) || value[DRAFT_STATE]
  ) return false;
...
function readPropFromProto(state, source, prop) {
  ...
  return desc
    ? VALUE in desc ? desc[VALUE] : desc.get?.call(state.draft_)
    : void 0;
}
...
//#endregion
export { NOTHING as t, produce as n };
