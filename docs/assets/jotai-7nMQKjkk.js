...
function shouldThrowSynchronously(error) {
  ...
  return (name === "RangeError" || name === "InternalError") &&
    (message.includes("call stack") || message.includes("too much recursion") ||
...
var BUILDING_BLOCK_atomRead = (_buildingBlocks, _store, atom, ...params) =>
  atom.read(...params);
var BUILDING_BLOCK_atomWrite = (_buildingBlocks, _store, atom, ...params) =>
  atom.write(...params);
var BUILDING_BLOCK_atomOnInit = (_buildingBlocks, store, atom) =>
...
var BUILDING_BLOCK_flushCallbacks = (buildingBlocks, store) => {
  ...
  if (
    !storeHooks.f && !changedAtoms.size && !mountCallbacks.size &&
...
function useStore(options) {
  ...
  return (options == null ? void 0 : options.store) || store ||
    getDefaultStore();
}
...
function useAtomValue(atom, options) {
  ...
  const [[valueFromReducer, storeFromReducer, atomFromReducer], rerender] = y(
    (prev) => {
